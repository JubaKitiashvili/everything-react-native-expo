...
(function () {
  ...
  const AGENT_DEFS = {
    ...
    'visual-debugger': { bodyColor: '#ec407a', traitColor: '#d81b60', trait: 'magnifier' },
    'documentation-generator': { bodyColor: '#78909c', traitColor: '#546e7a', trait: 'book' },
  };
  ...
  const CONFERENCE_SEATS = {
    ...
    'visual-debugger': { x: 722, y: 400 },
    'documentation-generator': { x: 722, y: 432 },
  };
  ...
  const drawCharacter = (ctx, ox, oy, def, headBob, typing, armOffset) => {
    ...
    ctx.fillRect(ox + 8, oy + 29, 16, 3);

    // Shoes
    ctx.fillStyle = '#1a1a1a';
    ctx.fillRect(ox + 10, oy + 29, 5, 3);
    ctx.fillRect(ox + 17, oy + 29, 5, 3);
    ...
    ctx.fillRect(ox + 12, oy + 14, 8, 2);
    // Badge/pocket
    ctx.fillStyle = def.traitColor;
    ctx.fillRect(ox + 10, oy + 18, 3, 3);
    ...
    // Ear
    ctx.fillStyle = '#EAA89B';
    ctx.fillRect(ox + 9, oy + 8 + hb, 2, 4);
    ctx.fillRect(ox + 21, oy + 8 + hb, 2, 4);

    // Hair (with highlight)
    ctx.fillStyle = HAIR;
    ctx.fillRect(ox + 9, oy + 2 + hb, 14, 5);
    ctx.fillRect(ox + 9, oy + 4 + hb, 2, 6);
    ctx.fillRect(ox + 21, oy + 4 + hb, 2, 4);
    // Hair highlight
    ctx.fillStyle = '#5D4A3C';
    ctx.fillRect(ox + 11, oy + 3 + hb, 4, 2);

    // Eyes (with white sclera)
    ctx.fillStyle = '#fff';
    ctx.fillRect(ox + 12, oy + 8 + hb, 4, 3);
    ctx.fillRect(ox + 17, oy + 8 + hb, 4, 3);
    // Pupils
    ctx.fillStyle = '#222';
    ctx.fillRect(ox + 13, oy + 9 + hb, 2, 2);
    ctx.fillRect(ox + 18, oy + 9 + hb, 2, 2);
    // Eye shine
    ctx.fillStyle = '#fff';
    ctx.fillRect(ox + 14, oy + 8 + hb, 1, 1);
    ctx.fillRect(ox + 19, oy + 8 + hb, 1, 1);
  ...
  const drawWalkingCharacter = (ctx, ox, oy, def, legOffset) => {
    ...
    // Shoes
    ctx.fillStyle = '#1a1a1a';
    ctx.fillRect(ox + 10, oy + 29 + lo, 5, 3);
    ctx.fillRect(ox + 17, oy + 29 - lo, 5, 3);
    ...
    // Hair
    ctx.fillStyle = HAIR;
    ctx.fillRect(ox + 9, oy + 2, 14, 5);
    ctx.fillRect(ox + 9, oy + 4, 2, 6);
    ctx.fillRect(ox + 21, oy + 4, 2, 4);
    ctx.fillStyle = '#5D4A3C';
    ctx.fillRect(ox + 11, oy + 3, 4, 2);

    // Eyes
    ctx.fillStyle = '#fff';
    ctx.fillRect(ox + 12, oy + 8, 4, 3);
    ctx.fillRect(ox + 17, oy + 8, 4, 3);
    ctx.fillStyle = '#222';
    ctx.fillRect(ox + 13, oy + 9, 2, 2);
    ctx.fillRect(ox + 18, oy + 9, 2, 2);
    ctx.fillStyle = '#fff';
    ctx.fillRect(ox + 14, oy + 8, 1, 1);
    ctx.fillRect(ox + 19, oy + 8, 1, 1);
  ...
  const drawTrait = (ctx, ox, oy, def) => {
    switch (def.trait) {
      ...
      case 'magnifier':
        ...
        ctx.fillStyle = def.traitColor;
        ctx.fillRect(ox + 24, oy + 3, 6, 6);
        ctx.fillStyle = '#e0f7fa';
        ctx.fillRect(ox + 25, oy + 4, 4, 4);
        // Handle
        ctx.fillStyle = def.traitColor;
        ctx.fillRect(ox + 29, oy + 8, 2, 4);
        break;
      case 'book':
        // Open book
        ctx.fillStyle = def.traitColor;
        ctx.fillRect(ox + 23, oy + 4, 8, 6);
        ctx.fillStyle = '#fff';
        ctx.fillRect(ox + 24, oy + 5, 3, 4);
        ctx.fillRect(ox + 28, oy + 5, 3, 4);
        // Spine
        ctx.fillStyle = def.bodyColor;
        ctx.fillRect(ox + 27, oy + 4, 1, 6);
        break;
    }
  };
  ...
  const drawAgentSprites = (ctx) => {
    ...
    for (const [name, sprite] of Object.entries(agentSprites)) {
      ...
      const row = sprite.isMoving ? STATUS_TO_ROW.moving : STATUS_TO_ROW[sprite.status] || 0;
      ...
      ctx.drawImage(
        ...
        FRAME_SIZE,
        FRAME_SIZE,
        sprite.x - FRAME_SIZE / 2,
        sprite.y - FRAME_SIZE / 2,
        FRAME_SIZE,
        FRAME_SIZE,
      );

      // Status dot
      const dotColor = sprite.isMoving ? '#FF9800' : STATUS_DOT_COLORS[sprite.status] || '#9E9E9E';
      ...
      ctx.textAlign = 'center';
      const shortName =
        (window.OfficeCanvas.SHORT_NAMES && window.OfficeCanvas.SHORT_NAMES[name]) || name;
