...
async function executePipeline({ ticket, config, provider, auditData, stackInfo, logger }) {
  ...
  const baseBranch = (config.repo && config.repo.base_branch) || 'main';
  const worktree = createWorktree(repoPath, taskVars.branch, logger, baseBranch);
  if (!worktree.path) {
  ...
  try {
    ...
    // 5i. Push branch and create PR
    publishDashboardEvent('worker:step', { taskId: ticket.id, step: 'pr' });
    let prUrl = null;
    try {
      // Push the branch to remote
      execSync(`git push -u origin ${taskVars.branch}`, {
        cwd: worktree.path,
        stdio: 'pipe',
        timeout: 60000,
      });
      logger.info(`Pushed branch ${taskVars.branch} to origin`);

      // Create PR with gh CLI
      const testStatus = testsFailing ? 'FAILING' : 'PASSING';
      const prBody = [
        `## Summary`,
        ``,
        `Automated implementation for: **${ticket.title}**`,
        ``,
        `**Agent:** ${agent}`,
        `**Confidence:** ${confidenceResult.score}/100 (${confidenceResult.level})`,
        `**Tests:** ${testStatus}`,
        `**Self-review:** ${reviewResult.issues.length} issue(s) found`,
        ``,
        `## Ticket`,
        ...
        ``,
        `---`,
        `Generated by ERNE Worker`,
      ].join('\n');

      const baseBranch = (config.repo && config.repo.base_branch) || 'main';
      const prTitle = `${ticket.type === 'bug' ? 'fix' : 'feat'}: ${ticket.title}`;
      const prResult = execSync(
        `gh pr create --title "${prTitle.replace(/"/g, '\\"')}" --body "${prBody.replace(/"/g, '\\"')}" --base ${baseBranch}`,
        { cwd: worktree.path, stdio: 'pipe', timeout: 30000, encoding: 'utf-8' },
      );
      prUrl = prResult.trim();
      logger.info(`Created PR: ${prUrl}`);
    } catch (err) {
      logger.warn(`PR creation failed: ${err.message}`);
    }
