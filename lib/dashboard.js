...
const { getRegisteredPort, findFreePort, registerPort, unregisterPort } = require('../scripts/hooks/lib/port-registry');
  ...
  try {
    if (fs.existsSync(SETTINGS_LOCAL_PATH)) {
      ...
      if (settingsLocal.hooks) {
        ...
        const hasPre = preToolUse.some(entry =>
          entry.matcher === 'Agent' && entry.hooks && entry.hooks.some(h =>
            h.command && h.command.includes('dashboard-event.js')
          )
        );
        const hasPost = postToolUse.some(entry =>
          entry.matcher === 'Agent' && entry.hooks && entry.hooks.some(h =>
            h.command && h.command.includes('dashboard-event.js')
          )
        );
...
function _addDashboardHooksToSettingsLocal() {
  let settingsLocal = {};
  if (fs.existsSync(SETTINGS_LOCAL_PATH)) {
    try { settingsLocal = JSON.parse(fs.readFileSync(SETTINGS_LOCAL_PATH, 'utf8')); } catch { /* fresh */ }
  }
